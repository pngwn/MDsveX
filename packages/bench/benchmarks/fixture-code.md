...
function tokenize(source: string): Token[] {
  ...
  while (i < len) {
    ...
    if ((ch >= 65 && ch <= 90) || (ch >= 97 && ch <= 122) || ch === 95) {
      ...
      tokens.push({ kind: TokenKind.Ident, start, end: i, value: source.slice(start, i) });
      continue;
    }
    ...
    if (ch >= 48 && ch <= 57) {
      const start = i;
      while (i < len && source.charCodeAt(i) >= 48 && source.charCodeAt(i) <= 57) i++;
      if (i < len && source.charCodeAt(i) === 46) {
        i++;
        while (i < len && source.charCodeAt(i) >= 48 && source.charCodeAt(i) <= 57) i++;
      }
      tokens.push({ kind: TokenKind.Number, start, end: i, value: Number(source.slice(start, i)) });
      continue;
    }
    ...
    if (ch === 34 || ch === 39) {
      ...
      tokens.push({ kind: TokenKind.String, start, end: i, value: source.slice(start + 1, i - 1) });
      continue;
    }
...
```typescript
function parseExpr(tokens: Token[], pos: number, minBP: number): [Expr, number] {
