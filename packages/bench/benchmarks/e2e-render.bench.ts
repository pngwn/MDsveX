...
import { PFMParser, WireEmitter, PluginDispatcher, parse_markdown_svelte } from "@mdsvex/parse";
...
const satteri_autolink = defineMdastPlugin({
  name: "autolink",
	heading(node, ctx) {
		ctx.replaceNode(node, {
			...
			children: [
				{
					type: "link",
					url: `#${slugify(ctx.textContent(node))}`,
...
function pfm_autolink(): ParsePlugin {
	return {
		heading: {
			parse(node) {
...
// function pfm_e2e(source: string, plugins: ParsePlugin[] = []): string {
// 	const tree = new TreeBuilder(source.length >> 3 || 128, new PluginDispatcher(plugins));
...
for (const [name, source] of Object.entries(fixtures)) {
	describe(`e2e: ${name} (${source.length} bytes)`, () => {
		...
		bench("pfm(autolinks)", () => {
			parse_markdown_svelte(source, { plugins: [pfm_autolink()] })
		});
		...
		bench("satteri (satteri_autolink)", () => {
			markdownToHtml(source, { mdastPlugins: [satteri_autolink]});
		});

		bench("satteri (mdast + hast)", () => {
			markdownToHtml(source, { mdastPlugins: [satteri_autolink] , hastPlugins: [addLinkClasses]});
		});
	});
}
