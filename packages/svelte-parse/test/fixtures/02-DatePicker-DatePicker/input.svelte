...
function onDatePickerChange(selectedDates) {
	let _date = selectedDates[0]
		? datePicker.formatDate(selectedDates[0], datePicker.config.dateFormat)
		: null;
	let _toDate = selectedDates[1]
		? datePicker.formatDate(selectedDates[1], datePicker.config.dateFormat)
		: null;
...
function prevDay() {
	...
	const prev = dayjs(date, "DD/MM/YYYY")
		.subtract(1, "day")
		.format("DD/MM/YYYY");
...
function createDatePicker() {
	const defaultDate =
		defaultToday || canNavigate ? dayjs().format("DD/MM/YYYY") : null;
...
onDestroy(async () => {
	if (picker) {
		picker
			...
			.removeEventListener("focus", onInputFocus);
		picker
			.querySelector(".form-control")
			.removeEventListener("blur", onInputBlur);
	}
