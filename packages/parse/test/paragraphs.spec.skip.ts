...
const expect_paragraph_nodes = (
	...
	for (let index = 0; index < tokens.size; index += 1) {
		if (tokens.kind_at(index) === node_kind.text) {
...
describe('paragraphs', () => {
	...
	test('pfm example 220 keeps single blank line as paragraph separator', () => {
		...
		expect(to_array(tokens.starts_slice())).toEqual([
			0, 3, 4, 7, 8, 9, 12, 13, 16,
		]);
		expect(to_array(tokens.ends_slice())).toEqual([
			3, 4, 7, 8, 9, 12, 13, 16, 17,
		]);
