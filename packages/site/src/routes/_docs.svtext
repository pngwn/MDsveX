...
By default `mdsvex` will optimise your code blocks by placing them in a Svelte `@html` block. To disable this behaviour either because you love chaos or you are using `mdsvex` without Svelte (perhaps via the `compile` function), set `optimise: false`
