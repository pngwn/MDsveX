...
export type Op =
	| { op: 'open'; id: number; kind: number; kindName: string; start: number; parent: number; extra: number; pending: boolean }
...
export class RecordingEmitter implements Emitter {
	...
	open(id: number, kind: number, start: number, parent: number, extra: number, pending: boolean): void {
		this.ops.push({ op: 'open', id, kind, kindName: kind_to_string(kind), start, parent, extra, pending });
	}
